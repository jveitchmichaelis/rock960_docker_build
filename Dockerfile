...
RUN apt-get install -qq binfmt-support qemu-user-static python-dbus python-debian python-parted python-yaml apt-utils wget cpio bzip2 gdisk
...
RUN cd rock960dev && repo start rock960-dev --all
...
# Can't do this because we can't do privilged shit inside Docker..
